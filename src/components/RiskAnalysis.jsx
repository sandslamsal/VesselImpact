      ...
      <section className="toolbar-card">
        <div className="results-head">
          ...
          <div>
            ...
            <p>
              <TexText text="Probability-based risk analysis per AASHTO LRFD Article 3.14.5: $AF = (N)(PA)(PG)(PC)(PF)$ for one vessel classification striking one bridge component. Sum $AF$ over all vessel classes and components and compare with 0.001 for typical bridges or 0.0001 for critical or essential bridges." />
            </p>
          </div>
        </div>
        ...
        <div className="field-grid">
          ...
          {region !== 'straight' ? (
            <NumberField label="Turn / bend angle $\theta$" unit="°" value={theta} onChange={setTheta} />
          ) : null}
          <NumberField label="Parallel current $V_C$" unit="knots" value={Vc} onChange={setVc} />
          <NumberField label="Cross-current $V_{XC}$" unit="knots" value={Vxc} onChange={setVxc} />
        ...
        <div className="field-grid">
          ...
          <NumberField label="Vessel impact force $P$" unit="kip" value={P} onChange={setP} hint="$P_S$, $P_{BH}$, $P_{DH}$, or $P_{MT}$ from the force tabs" />
        ...
        <div className="field-grid">
          ...
          <NumberField label="Min. velocity $V_{MIN}$" unit="ft/s" value={Vmin} onChange={setVmin} />
        </div>
      </section>
      ...
      {res ? (
        <>
          ...
          <div className="status-banner verdict-banner">
            <Verdict pass={res.passTypical} label={`Typical bridge:  $AF ${res.passTypical ? '\\le' : '>'} 0.001$  ($AF$ = ${fmt(res.AF, 6)})`} />
            <Verdict pass={res.passCritical} label={`Critical / essential bridge:  $AF ${res.passCritical ? '\\le' : '>'} 0.0001$`} />
            <span className="verdict-note">
              <TexText text="Acceptance applies to the $AF$ summed over all vessel classes and exposed components (Art. 3.14.5) — this result is for one component and one vessel class." />
            </span>
          </div>
          ...
          <section className="results-card">
            <div className="results-head">
              ...
              <div>
                ...
                <p>
                  <TexText text="Normal distribution of the aberrant sailing path ($\sigma = LOA$) with the geometric-probability area across the impact zone, and the design collision velocity distribution." />
                </p>
              </div>
            </div>
        ...
        <div className="status-banner"><TexText text="Enter valid traffic and geometry ($N$ and $LOA$ greater than zero) to compute the annual frequency." /></div>
      )}
    </>
  )
}
